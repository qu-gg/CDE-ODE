    ...
    while model.epoch <= epochs:
        ...
        for i, b in tqdm.tqdm(enumerate(dl)):
            ...
            unique_idx, counts = np.unique(
                obs_idx.cpu().detach().numpy(), return_counts=True)
        ...
        with torch.no_grad():
            ...
            for i, b in enumerate(dl_val):
                ...
                loss_val += c_loss.cpu().detach().numpy()
                num_obs += 1
...
def compute_optimal_eval_loss(dl_val, stockmodel, delta_t, T):
    ...
    for i, b in enumerate(dl_val):
        times = b["times"]
        time_ptr = b["time_ptr"]
        X = b["X"].cpu().detach().numpy()
        start_X = b["start_X"].cpu().detach().numpy()
        obs_idx = b["obs_idx"].cpu().detach().numpy()
        n_obs_ot = b["n_obs_ot"].cpu().detach().numpy()
        num_obs += 1
...
def plot_one_path_with_pred(
    ...
    res = model.get_pred(times, time_ptr, X, obs_idx, delta_t, T,
                         start_X)
    path_y_pred = res['pred'].cpu().detach().numpy()
